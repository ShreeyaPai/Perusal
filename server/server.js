...
app.post("/signup", async (req, res) => {
  ...
  app.post("/search", async (req, res) => {
    ...
    try{
      const checkResult = await db.query(`SELECT username FROM users WHERE userid=(SELECT userid FROM ${db.escapeIdentifier(search)})`);
  ...
  app.post("/add", async (req, res) => {
    ...
    const username=req.body.username;
    try {
      const checkResult = await db.query(`SELECT userid FROM ${db.escapeIdentifier(badge)} NATURAL JOIN users WHERE users.username=$1`,[username]);
      if (checkResult.rows.length > 0) {
        res.status(400).send("Domain already chosen!");
      } else {
        try{
          const user=await db.query("SELECT userid FROM users WHERE username=$1",[username]); 
